...
const csvValido = `falante,texto,categorizacao
Terapeuta,Oi,
Paciente,Olá,`
...
describe('CsvService', () => {
  ...
  it('parseCsvToTranscription deve retornar transcrição, headers e records', () => {
    ...
    expect(resultado.headers).toEqual(['falante', 'texto', 'categorizacao'])
  ...
  it('parseCsvToTranscription deve lançar exceção se não houver dados válidos', () => {
    const csvSemDados = `falante,texto,categorizacao\n,,\n,,`
  ...
  it('reconstructCsvWithCategorization deve atualizar e converter para CSV', () => {
    ...
    const csvFinal = service.reconstructCsvWithCategorization(parseResult, categorizado)
    expect(csvFinal).toContain('"falante","texto","categorizacao"')
    expect(csvFinal).toContain('"Terapeuta","Oi","SRE"')
    expect(csvFinal).toContain('"Paciente","Olá","SA"')
  })

  it('reconstructCsvWithCategorization deve deixar categorizacao vazia se não encontrar correspondência', () => {
    const parseResult = service.parseCsvToTranscription(csvValido)
    const categorizado = [
      { falante: 'Terapeuta', texto: 'Oi', categoria: 'SRE' },
    ]
    const csvFinal = service.reconstructCsvWithCategorization(parseResult, categorizado)
    expect(csvFinal).toContain('"Paciente","Olá",""')
  })
})