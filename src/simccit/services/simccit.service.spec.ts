...
describe('SimccitService', () => {
  ...
  beforeEach(() => {
    service = new SimccitService(
      mockHttpService as any as HttpService,
      mockCsvService as any as CsvService,
    )
    // @ts-ignore
  ...
  describe('categorizeTranscript', () => {
    it('deve retornar os dados parseados da API', async () => {
      const fakeResponse = { data: { choices: [{ message: { content: '[{"falante":"Terapeuta","texto":"Oi","categoria":"SRE"}]' } }] } }
  ...
  describe('categorizeCsv', () => {
    it('deve parsear, categorizar e reconstruir o CSV', async () => {
      ...
      vi.spyOn(service, 'categorizeTranscript').mockResolvedValue([{ falante: 'Terapeuta', texto: 'Oi', categoria: 'SRE' }])
    ...
    it('deve relançar exceções conhecidas', async () => {
      mockCsvService.parseCsvToTranscription.mockImplementation(() => { throw new BadRequestException('bad') })
      await expect(service.categorizeCsv('csv')).rejects.toThrow(BadRequestException)
    })

    it('deve encapsular erros desconhecidos', async () => {
      mockCsvService.parseCsvToTranscription.mockImplementation(() => { throw new Error('other') })
      await expect(service.categorizeCsv('csv')).rejects.toThrow(InternalServerErrorException)
    })
  })
  ...
  describe('processApiResponse', () => {
    ...
    it('deve parsear saída válida', () => {
      const response = { data: { choices: [{ message: { content: '[{"falante":"Terapeuta","texto":"Oi","categoria":"SRE"}]' } }] } }
    ...
    it('deve usar fallback se parseApiOutput lançar exceção', () => {
      ...
      vi.spyOn(service as any, 'parseApiOutput').mockImplementation(() => { throw new Error('fail') })
      vi.spyOn(service as any, 'fallbackResponseParsing').mockReturnValue([{ falante: 'Terapeuta', texto: 'Oi', categoria: 'SRE' }])
      const result = service['processApiResponse'](response)
      expect(result).toEqual([{ falante: 'Terapeuta', texto: 'Oi', categoria: 'SRE' }])
    })
  })
