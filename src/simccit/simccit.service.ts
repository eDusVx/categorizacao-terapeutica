...
export class SimccitService {
    ...
    private validateJsonArrayStrict(output: string): FalaCategorizada[] {
        ...
        if (!trimmed.startsWith('[') || !trimmed.endsWith(']')) {
            throw new Error('A resposta não é um array JSON puro.')
        }
        ...
        if (!Array.isArray(data)) {
            throw new Error('A resposta não é um array JSON.')
        }

        for (const obj of data) {
            if (typeof obj !== 'object' || !('falante' in obj) || !('texto' in obj) || !('categoria' in obj)) {
                throw new Error('Um ou mais objetos não possuem as chaves obrigatórias.')
            }
        }
